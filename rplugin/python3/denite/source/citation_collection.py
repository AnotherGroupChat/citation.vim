...
sub_sources = [
    ...
    "duplicate_keys", "file", "isbn", "publication", "key", "key_inner",
    "language", "issue", "notes", "pages", "publisher", "tags", "title", "type",
...
class Source(Base):
  ...
  def _gather_items(self, context):
    ...
    if context['__field'] is 'key':
      text = (
          self.vars['key_outer_prefix'] + self.vars['key_inner_prefix'] + '{}' +
    ...
    # Retirn items
    for item in items:
      if (not self.vars['collection'] or
          self.vars['collection'] in item.collections):
        candidate = {
...
class Source(Base):
  ...
  def gather_candidates(self, context):
    ...
    for item in self._get_items(context):
      #raise Exception(item.describe(context))
      #for col in item.collections:
      #    if not col in collections:
      candidates.append(
          {
              "word":
                  f"[@{item.key}] | {item.nick} | {item.title}",
              "title":
                  f"{item.title}",
              "nick": f"{item.nick}",
              "action__text":
                  f"\\cite{{{item.key}}}",
              "action__path":
                  f"/home/dylan/phd/notes/reviews/{item.nick or item.key}.md",
          })
    return candidates
